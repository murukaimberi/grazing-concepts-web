...
describe('table tests', () => {
  ...
  let table = {
    ...
    startDeviceActivity: [{
      args: ['a', 'b', 'c', 'd'],
      params: {appPackage: 'a', appActivity: 'b', appWaitPackage: 'c', appWaitActivity: 'd'}
    },
    ...
    installApp: {
      args: [path.resolve(__dirname, 'totally_real_apk.apk')],
        ...
        driver[commandName].apply(driver, testcase.args || []).then((results) => {
          ...
          if (testcase.session) {
            for (let varname in testcase.session) {
              expect(results.session[varname]).toEqual(testcase.session[varname]);
            }
          }
          if (testcase.params) {

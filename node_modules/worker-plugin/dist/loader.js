...
function pitch(request) {
  ...
  var plugin = compilerOptions.plugins.find(function (p) {
    return p[WORKER_PLUGIN_SYMBOL];
  }) || {};
  ...
  var plugins = (pluginOptions.plugins || []).map(function (plugin) {
    ...
    var found = compilerOptions.plugins.find(function (p) {
  ...
  workerCompiler.runAsChild(function (err, entries, compilation) {
    ...
    if (!err && !entry) {
      err = Error(("WorkerPlugin: no entry for " + request));
    }
    if (err) {
      return cb(err);
    }

...
function filterFiles(files, patterns, include) {
  ...
  var matcher = patterns.map(function (pattern) {
    return new minimatch_1.Minimatch(pattern, {dot: !include});
  }); // `glob` always enables `dot` for ignore patterns
  return files.filter(function (file) {
    return include === matcher.some(function (pattern) {
...
function resolveFilesAndProgram(_a, logger) {
  ...
  if (files.length === 0) {
    ...
    filesFound = filterFiles(program.getSourceFiles().map(function (f) {
      return f.fileName;
    }), files, true);

...
function walk(context) {
  ...
  function getApparentJsDoc(node, docType, sourceFile) {
    ...
    return equivalentNodesForDocs
      .map(function (docsNode) {
        return tsutils.getJsDoc(docsNode, sourceFile);
      })
      .filter(function (nodeDocs) {
        return nodeDocs !== undefined;
      })
      .reduce(function (docs, moreDocs) {
        return tslib_1.__spreadArrays(docs, moreDocs);
      }, []);
  }
  ...
  function getEquivalentNodesForDocs(node, docType) {
    ...
    if (tsutils.isConstructorDeclaration(node)) {
      ...
      return members.filter(function (child) {
        return tsutils.isConstructorDeclaration(child);
      });
    }
  ...
  if (node.modifiers !== undefined) {
    description += node.modifiers
      .map(function (modifier) {
        return describeModifier(modifier.kind);
      })
      .join(" ") + " ";
  }

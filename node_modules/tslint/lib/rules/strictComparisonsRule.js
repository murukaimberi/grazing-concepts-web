...
var Rule = /** @class */ (function (_super) {
  ...
  Rule.INVALID_TYPES = function (types1, types2) {
    var types1String = types1.map(function (type) {
      return typeNames[type];
    }).join(" | ");
    var types2String = types2.map(function (type) {
      return typeNames[type];
    }).join(" | ");
...
function getStrictestComparableType(typesLeft, typesRight) {
  var overlappingTypes = typesLeft.filter(function (type) {
    return typesRight.indexOf(type) >= 0;
  });
...
function arrayContainsKind(types, typeToCheck) {
  return types.some(function (type) {
    return typeToCheck.indexOf(type) >= 0;
  });
}

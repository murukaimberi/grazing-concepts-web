...
var isAttributeDecorator = function (decorator) {
  return utils_1.getDecoratorName(decorator) === ATTRIBUTE;
};
var validateConstructor = function (walkContext, node) {
  node.parameters.forEach(function (parameter) {
    return validateParameter(walkContext, parameter);
  });
};
...
var validateParameter = function (walkContext, node) {
  typescript_1.createNodeArray(node.decorators).forEach(function (decorator) {
    return validateDecorator(walkContext, decorator);
  });
};

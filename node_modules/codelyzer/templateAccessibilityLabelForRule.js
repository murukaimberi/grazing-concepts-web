...
var Rule = (function (_super) {
  ...
  Rule.prototype.areOptionsValid = function () {
    ...
    return (ruleArgumentsKeys.every(function (argumentKey) {
        return propertiesKeys.includes(argumentKey);
      }) &&
      ruleArgumentsKeys
        ...
        .every(function (argumentValue) {
          return Array.isArray(argumentValue) && argumentValue.length > 0;
        }));
  };
  Rule.metadata = {
...
var TemplateVisitorCtrl = (function (_super) {
  ...
  function TemplateVisitorCtrl(sourceFile, options, context, templateStart) {
    ...
    var _a = (options.ruleArguments[0] || {}), controlComponents = _a.controlComponents,
      labelAttributes = _a.labelAttributes, labelComponents = _a.labelComponents;
  ...
  TemplateVisitorCtrl.prototype.hasControlComponentInsideElement = function (element) {
    return Array.from(this.controlComponents).some(function (controlComponentName) {
      return isChildNodeOf_1.isChildNodeOf(element, controlComponentName);
    });
  };
  TemplateVisitorCtrl.prototype.hasValidAttrOrInput = function (element) {
    var _this = this;
    return __spreadArrays(element.attrs, element.inputs).map(function (attrOrInput) {
      return attrOrInput.name;
    })
      .some(function (attrOrInputName) {
        return _this.labelAttributes.has(attrOrInputName);
      });
  };

...
var SelectorPropertyBase = (function (_super) {
  ...
  function SelectorPropertyBase(options) {
    ...
    _this.types = utils_1.arrayify(ruleArguments[0] || [exports.OPTION_TYPE_ATTRIBUTE, exports.OPTION_TYPE_ELEMENT]).reduce(function (previousValue, currentValue) {
      return previousValue.concat(SELECTOR_TYPE_MAPPER[currentValue]);
    }, []);
    return _this;
  }
  ...
  SelectorPropertyBase.prototype.validatePrefixes = function (selectors) {
    ...
    return selectors.some(function (selector) {
      return _this.prefixes.some(function (prefix) {
        return selectorValidator_1.SelectorValidator.prefix(prefix, _this.style)(selector);
      });
    });
  };
...
var validateClassDeclaration = function (context, node) {
  return typescript_1.createNodeArray(node.decorators).forEach(function (decorator) {
    return validateDecorator(context, decorator);
  });
};

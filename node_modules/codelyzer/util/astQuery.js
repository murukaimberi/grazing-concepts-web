...
function callExpression(dec) {
  return function_1.Maybe.lift(dec.expression).fmap(function (expr) {
    return (expr && ts.isCallExpression(expr) ? expr : undefined);
  });
}
...
function objectLiteralExpression(expr) {
  return function_1.Maybe.lift(expr.arguments[0]).fmap(function (arg0) {
...
function withIdentifier(identifier) {
  return function_1.ifTrue(function (expr) {
    return ts.isIdentifier(expr.expression) && expr.expression.text === identifier;
  });
}
...
function getStringInitializerFromProperty(propertyName, ps) {
  ...
  return (getInitializer(property)
    .fmap(function (expr) {
      return (expr && utils_1.isStringLiteralLike(expr) ? expr : undefined);
    }));
}

...
function getAnimations(dec) {
  return astQuery_1.decoratorArgument(dec).bind(function (expr) {
    var property = expr.properties.find(function (p) {
      return astQuery_1.isProperty('animations', p);
    });
    return astQuery_1.getInitializer(property).fmap(function (expr) {
      return (ts.isArrayLiteralExpression(expr) ? expr : undefined);
    });
  });
}
...
function getInlineStyle(dec) {
  return astQuery_1.decoratorArgument(dec).bind(function (expr) {
    ...
    return astQuery_1.getInitializer(property).fmap(function (expr) {
      return (expr && ts.isArrayLiteralExpression(expr) ? expr : undefined);
    });
  });
}

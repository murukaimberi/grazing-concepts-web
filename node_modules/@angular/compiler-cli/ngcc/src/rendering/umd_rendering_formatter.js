(function (factory) {
  ...
  function renderCommonJsDependencies(output, wrapperFunction, imports) {
    ...
    var importString = imports.map(function (i) {
      return "require('" + i.specifier + "')";
    }).join(',');
  ...
  function renderGlobalDependencies(output, wrapperFunction, imports) {
    ...
    var importString = imports.map(function (i) {
      return "global." + getGlobalIdentifier(i);
    }).join(',');
  ...
  function renderFactoryParameters(output, wrapperFunction, imports) {
    ...
    var parameterString = imports.map(function (i) {
      return i.qualifier;
    }).join(',');
  ...
  function isCommonJSConditional(value) {
    ...
    if (!oneOfBinaryConditions(value.condition, function (exp) {
      return isTypeOf(exp, 'exports', 'module');
    })) {
      return false;
    }
  ...
  function find(node, test) {
    return test(node) ? node : node.forEachChild(function (child) {
      return find(child, test);
    });
  }
